...
vector<Process> parseProcesses(int &maxMemory, int &numProcesses, int &globalCPUAllocated, int &contextSwitchTime)
{
    ...
    for (int p = 0; p < numProcesses; p++)
    {
        ...
        int opcode, operand;
        int instructionsRead = 0; // Track the number of instructions read
        ...
        while (instructionsRead < proc.numInstructions)
        {
            ...
            int numOperands = 0;
            if (opcode == 1)
                numOperands = 2; // Compute: two operands
            else if (opcode == 2)
                numOperands = 1; // Print: one operand
            else if (opcode == 3)
                numOperands = 2; // Store: two operands
            else if (opcode == 4)
                numOperands = 1; // Load: one operand
...
void allocateMemory(vector<int> &mainMemory, const vector<Process> &processes, int maxMemory, vector<int> &pcbLocations)
{
    ...
    for (const Process &proc : processes)
    {
        ...
        for (size_t i = 0; i < proc.instructions.size();)
        {
            ...
            int numOperands = (opcode == 1) ? 2 : (opcode == 2) ? 1
                                              : (opcode == 3)   ? 2
                                              : (opcode == 4)   ? 1
                                                                : 0;
...
int main()
{
    ...
    // After initializing readyQueue:
    int totalCpuCycles = 0;
    while (!readyQueue.empty())
    {
        int pcbAddr = readyQueue.front();
        readyQueue.pop();
        executeProcess(mainMemory, pcbAddr, totalCpuCycles);
    }
